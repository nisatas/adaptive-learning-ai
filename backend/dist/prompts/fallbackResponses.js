...
exports.fallbackStudentFeedbackResponse = fallbackStudentFeedbackResponse;
exports.fallbackStudentAnalysisResponse = fallbackStudentAnalysisResponse;
exports.fallbackQuizBehaviorResponse = fallbackQuizBehaviorResponse;
exports.fallbackTeacherDashboardResponse = fallbackTeacherDashboardResponse;
exports.fallbackMeetWorkflowResponse = fallbackMeetWorkflowResponse;
exports.fallbackSupportPlanWorkflowResponse = fallbackSupportPlanWorkflowResponse;
exports.fallbackWeeklyReportWorkflowResponse = fallbackWeeklyReportWorkflowResponse;
function greetingName(name) {
    return name?.trim() ? name.trim() : 'Merhaba';
}
function focusFromInput(topic, mostDifficultTopic) {
    return mostDifficultTopic?.trim() || topic;
}
function fallbackStudentFeedbackResponse(input) {
    ...
    const belowAverage = input.classAverage !== undefined && score < input.classAverage;
    let shortFeedback;
    let nextStep;
...
function fallbackStudentAnalysisResponse(input) {
    const focus = focusFromInput(input.topic, input.mostDifficultTopic);
    const performanceLevel = input.quizScore >= 85 ? 'high' : input.quizScore >= 60 ? 'medium' : 'low';
    return {
        ...
        difficultySignal: input.quizScore < 60 ? 'high' : input.quizScore < 80 ? 'medium' : 'low',
        strengths: input.quizScore >= 70
            ? [`${focus} konusunda doğru cevap oranı destekleyici.`]
            : [`${focus} konusunda tamamlanan quiz verisi mevcut.`],
        needsSupport: input.quizScore < 70
...
function fallbackQuizBehaviorResponse(input) {
    return {
        interactionSummary: 'Analiz sınırlı veriyle oluşturuldu. Quiz sırasında etkileşim sinyalleri birlikte değerlendirildi.',
        attentionSignal: 'medium',
...
function fallbackTeacherDashboardResponse(input) {
...
function fallbackMeetWorkflowResponse(input) {
    const focus = focusFromInput(input.topic, input.mostDifficultTopic);
    return {
        ...
        parentOrGuardianNote: 'Görüşme öğrenme desteği amaçlıdır; performans gelişimi birlikte ele alınacaktır.',
        dashboardSummary: `${input.studentName} için meet planı hazırlandı.`,
        confidence: 'low',
    };
}
function fallbackSupportPlanWorkflowResponse(input) {
    const focus = focusFromInput(input.topic, input.mostDifficultTopic);
    return {
        ...
        studentMotivationMessage: 'Kısa ve düzenli tekrarlar bir sonraki quizde sana yardımcı olabilir.',
        adaptiveUiSuggestion: {
...
function fallbackWeeklyReportWorkflowResponse(input) {
    let progressTrend = 'stable';
    if (input.previousWeekAverage !== undefined &&
        input.classAverage > input.previousWeekAverage + 2) {
        progressTrend = 'improving';
    }
    else if (input.previousWeekAverage !== undefined &&
        input.classAverage < input.previousWeekAverage - 2) {
...
//# sourceMappingURL=fallbackResponses.js.map